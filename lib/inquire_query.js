        ...
        inquirer
            ...
            .then((ans) => {
                ...
                WHERE id =?;`,
                    ...
                    (err, rows) => {
                        if (err) throw err;
                        chosenRoleDept.id = rows[0].department_id;
                        return chosenRoleDept;
                    });
                ...
                db.query(sql,
                    ...
                    (err, result) => {
                        if (err) throw err;
                        console.info(`
                        The role '${chosenRole.name}', that belonged to the department ${chosenRoleDept.id}, was deleted from record.
...
function startCLI() {
    ...
    return inquirer
        ...
        .then((answer) => {
            ...
            if (answer.main === options[3]) addDepartment();
            if (answer.main === options[4]) addRegRole();
            if (answer.main === options[5]) addRegEmployee();
            if (answer.main === options[6]) updateRegEmployee();
            if (answer.main === options[7]) updateEmployeeManagers();
            if (answer.main === options[8]) deleteRegEmployee();
            if (answer.main === options[9]) deleteDepartment();
            if (answer.main === options[10]) deleteRegRole();
            if (answer.main === options[11]) {
                db.end();
